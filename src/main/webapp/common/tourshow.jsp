...
<script type="text/javascript">
    ...
    function loadgrid(){
        $('#grid').datagrid({
            title : '客户信息一览表',
            url:'http://localhost:8080/json/showtour.json',
            method: 'post',
            ...
            toolbar: [
                "-", {id: 'add', text: '组织自己的旅程',	iconCls: 'icon-add', handler: function () { add()} },
                ...
                "-", {id: 'exit',  text: '退出',iconCls: 'icon-exit', handler: function () {exit()}},
                "-", {id: 'showactivities',  text: '显示所有活动',iconCls: 'icon-exit', handler: function () {showActs()}
    ...
    function showActs() {
        ...
        if(row == null){
            ...
            $.ajax({
                type: "POST",
                url: "http://localhost:8080/tourController/getTourInfo",
                data: {"participate":show},
            });
    ...
    function reachShow()
    {window.location.href="http://localhost:8080/tourController/showActs";}
    ...
    function exit() {
        ...
        if(row == null){
            ...
            te=row.tourname+"|,|"+row.leader+"|,|"+row.introduction+"|,|"+row.startdate+"|,|"+row.enddate;
            $.ajax({
                type: "POST",
                url: "http://localhost:8080/tourController/getTourInfo",
                data: {"participate":te},
    ...
    function reachexit() {
        window.location.href="http://localhost:8080/tourController/exitTour";
    }
    function reachparticipate() {
        window.location.href="http://localhost:8080/tourController/participateTour";
    }
    /* 修改数据*/
    function edit() {
        var row = $('#grid').datagrid('getSelected'); //// 得到选中的一行数据
    ...
    function reload(){
        $.ajax({
            type: "POST",
            url: "http://localhost:8080/tourController/viewTour",
            data: null,
        });
        rere();
    ...
    function rere() {
        window.location.href="http://localhost:8080/common/tourshow.jsp";
    }
