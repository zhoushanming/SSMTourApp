...
@WebServlet(urlPatterns = "/servlet/downloadActs")
public class DownloadActsServlet extends HttpServlet {
    ...
    public void sendExcel(HttpServletRequest request,HttpServletResponse response){
        ...
        //excel标题
        String[] title = {"旅程名称","活动名称","活动内容","参与用户"};
        //excel文件名
        String fileName = "“"+tourname+"”旅程活动及人员表.xls";
        ApplicationContext act =
                new ClassPathXmlApplicationContext("applicationContext.xml");
        ActivityServiceImpl activityService=(ActivityServiceImpl)act.getBean("activityService");
        List<Activity> list=activityService.findActivityByTourname(tourname);
        ...
        String sheetName = "“"+tourname+"”的活动清单";
...
//                content[i][5]=usertem.getDept();
            activity=list.get(i);
            content[i][0]=activity.getTourname();
            content[i][1]=activity.getActivityname();
            content[i][2]=activity.getContent();
            content[i][3]=activity.getUsername();
        }
